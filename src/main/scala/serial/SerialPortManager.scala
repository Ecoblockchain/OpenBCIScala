...
package org.openbci.serial {
 ...
 object SerialPortManager {
   ...
   final val ENCODING = StandardCharsets.US_ASCII
 ...
 class SerialPortManager(private val portName: String, private val packetSize: Int = 33) {
  ...
  private val purgedMask = SerialPort.PURGE_RXCLEAR | SerialPort.PURGE_TXCLEAR |
  SerialPort.PURGE_RXABORT | SerialPort.PURGE_TXABORT
  private val eventsMask = SerialPort.MASK_RXCHAR + SerialPort.MASK_RXFLAG + SerialPort.MASK_TXEMPTY +
  SerialPort.MASK_CTS + SerialPort.MASK_DSR + SerialPort.MASK_ERR + SerialPort.MASK_BREAK
  ...
  if(!serialPort.setParams(RFDuinoUSB.baud, RFDuinoUSB.dataBits,
    RFDuinoUSB.stopBits, RFDuinoUSB.parity)) {
      close
      throw new SerialPortException(portName, "SerialPortManager",
        SerialPortException.TYPE_PARAMETER_IS_NOT_CORRECT)
    }

  serialPort.purgePort(purgedMask)
  serialPort.setEventsMask(eventsMask)

  def listen(events: LinkedBlockingQueue[Array[Byte]]): Unit =
    if(serialPort.isOpened) {
      serialPort.purgePort(purgedMask)
      serialPort.addEventListener(SerialPortReader(serialPort, events))
    }
